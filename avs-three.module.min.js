...
function ZoomRectangleInteractor( domElement ) {
	...
	this.setObjectScene = function( object, scene ) {
		...
		if ( object.userData.zoomRectangleTriggers ) {

			pointerTriggers = JSON.parse( object.userData.zoomRectangleTriggers );

		}

		zoomInMaxLimit = object.userData.zoomInMaxLimit;
		zoomOutMaxLimit = object.userData.zoomOutMaxLimit;
	...
	var zoom = function() {
		...
		return function( left, right, top, bottom ) {
			...
			var ndcX = ( ( left + right ) / scope.scene.viewportWidth ) - 1;
			var ndcY = 1 - ( ( top + bottom ) / scope.scene.viewportHeight );

			var scaleX = scope.scene.viewportWidth / ( right - left );
			var scaleY = scope.scene.viewportHeight / ( bottom - top );
	...
	function handleMouseMove( event ) {
		...
		scope.scene.rectCtx.clearRect( 0, 0, scope.scene.viewportWidth, scope.scene.viewportHeight );
		...
		scope.scene.rectCtx.setLineDash( [ 3 ] );
		scope.scene.rectCtx.strokeStyle = "#ff0000";
...
//		for ( var i = 0, l = this.interactors.length; i < l; i ++ ) {

//			this.interactors[ i ].removeEventListener( 'change', this.interactorUpdate.bind( this ) );

//			this.interactors[ i ].dispose();
...
//		this.interactors = [];
	...
	interactorUpdate: function( interactor ) {

		var object = interactor.object;

		object.traverse( function( child ) {

			if ( child.isBillboardText ) {

				child.scaleFactor = object.scale.y;
...
//        }
	...
	attachInteractors: function( object ) {
		...
		if ( object.userData.attachZoomRectangleInteractor ) {

			// Create ZoomRectangleInteract and attach to current object

			var zoomRectangleInteractor = new ZoomRectangleInteractor( this.domElement );

			this.addInteractor( zoomRectangleInteractor );

			zoomRectangleInteractor.setObjectScene( object, this.currentScene );
			zoomRectangleInteractor.addEventListener( 'change', this.interactorUpdate.bind( this, zoomRectangleInteractor ) );

		}

		if ( object.userData.attachPanInteractor && this.panInteractor ) {
...
function TransformInteractor( domElement ) {
	...
	this.setObjectScene = function( object, scene ) {
		...
		if ( object.userData.transformWheelActions ) {

			mouseWheelScrollActions = JSON.parse( object.userData.transformWheelActions );

		}

		if ( object.userData.transformTriggers ) {

			pointerTriggers = JSON.parse( object.userData.transformTriggers );
